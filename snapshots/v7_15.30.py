...
# Prior on a company's sensitivity to the rate, shrunk hard because the estimate carries almost no
# signal: across the warm-up histories its cross-case spread is entirely sampling noise (observed
# sd 0.029 against a mean standard error of 0.037), and splitting a single history in half the
# first half predicts the second with a slope of 0.08. The centre is the precision-weighted pooling
# of all of them, -0.024, tempered toward the one case whose true parameters are published, -0.018.
...
def _estimate_company_process(
    ...
    # taken literally extrapolates a lucky run into every option price. A plain Bayesian prior
    # leaves a long history nearly untouched and pulls a short trending one back hard.
