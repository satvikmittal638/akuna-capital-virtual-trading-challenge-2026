...
# ============================================================================
#
# Pricing is exact -- no Monte Carlo. A company's step is
# d log(V) = drift + rate_beta * dR + sector_beta * S + eps with `dR` the *realised* rate change, so
# over n steps it telescopes to rate_beta * (R_n - R_0) exactly, including the floor at zero where a
# down draw moves neither rate nor `dR`. Conditional on the terminal rate, (log A_n, log T_n) is then
# bivariate normal: mean log V_0 + n * drift + rate_beta * (r - R_0), per-step variance
# (sector_beta * sigma_S) ** 2 + idio ** 2, covariance sector_beta_A * sector_beta_T * sigma_S ** 2.
# So pricing is an exact Markov-chain DP over the terminal rate, then one normal CDF per rate state
# (a quadrature for two legs). Only (drift, rate_beta, v_A, v_T, cov) matter, and the rate chain only
# alpha_up = p_up + lam * target, alpha_down = p_down - lam * target -- hence the bot's `_Params`.
...
# Ridge penalty and reversion ceiling: a history spanning grid levels swamps the penalty.
_RATE_REVERSION_RIDGE: Final[float] = 8.0
_MAX_RATE_REVERSION: Final[float] = 0.35
# Prior on daily log drift. Positive, not zero: growing labs, near half a percent a day.
_DRIFT_PRIOR_MEAN: float = 0.005
_DRIFT_PRIOR_STD_DEV: float = 0.008
# Prior on their return correlation: both load on one sector shock, so it sits near three quarters.
_CORRELATION_PRIOR_MEAN: float = 0.75
_CORRELATION_PRIOR_STD_DEV: float = 0.20
# Prior on rate sensitivity, shrunk hard: no signal, cross-case spread is all sampling noise.
...
class _Params:
    ...
    # Posterior covariance of the OLS means, as multiples of the residual covariance given by
    # inv(X'X) for the design X = [1, dR]: (m00, m01, m11). Zero when parameters are exact.
    ...
    def effective_steps(self, num_steps: int, rate_change: float) -> float:
        ...
        Integrating a normal CDF over a normally distributed mean gives another normal CDF, so error
        in (drift, rate_beta) enters exactly as extra variance: for m = n * drift + rate_beta * dR,
        var(m) = [n, dR] inv(X'X) [n, dR]' times the residual covariance -- one multiple for both.
        """
...
def _estimate_rate_process(
    ...
    The true probabilities are linear in the rate level before clamping, so the intercept form comes
    from least squares on the move indicators sharing one slope: up(R) = alpha_up - lam * R and
    down(R) = alpha_down + lam * R. A down move at a zero rate looks like no move, so those rows
    leave the down regression only.
    """
    ...
    # Ridge, not plain least squares: a ten-day warm-up on two neighbouring grid points leaves no
    # variation and the raw slope explodes. The penalty fades once the history spans levels.
    ...
    # Laplace-style nudge so a run with no up (or down) move does not freeze that direction out.
...
def _estimate_company_process(
    ...
    # Shrink correlation: the error lands in spread pricing, where v_A + v_T - 2 cov is acute.
    ...
    # Few residuals means Student t, not normal: match the t variance, nu / (nu - 2), matrix-wide.
    company = {
    ...
    # Shrink drift and rate beta toward their priors, each by its own standard error.
...
class _CounterpartyStats:
    ...
    def markout_lower_bound(self, confidence_multiple: float) -> float:
        ...
        The *upper* end of the interval for our own profit, so a counterparty looks toxic only when the
        losses are too consistent to be luck.
        """
...
class MarketMaker:
    # The floor width before uncertainty. Split per path: `_half_spread` also sets the FOK hurdle.
    _QUOTE_BASE_HALF_SPREAD: Final[float] = 0.02
    _FOK_BASE_HALF_SPREAD: Final[float] = 0.05
    _MIN_HALF_SPREAD: Final[float] = 0.01
    # Capped inside what a fixed-width rival quotes: past a dime, width buys nothing anyone pays for.
    _MAX_HALF_SPREAD: Final[float] = 0.15
    # Winning is bad news -- they lift our offer when our error made it cheap -- so above zero, below one.
    ...
    # Fractions of capacity: contracts of typical margin the balance supports; 0.30 sent the rest away.
    ...
    _MAX_CHEAP_MULTIPLE: Final[float] = 3.0
    ...
    def __init__(
        ...
        # Every trade costs its max loss and only expiries repay it, so limits are set against capacity.
        ...
        # `FokOrder.order_type` is the counterparty's side, so accepting a BUY means selling, as
        # the RFQ pipeline does; `_match_pending_fok` checks it. (option_id, price, dir, qty).
    ...
    def _uncertainty(
        ...
        Each source is pushed by one standard error and the moves combined in quadrature, the errors
        being largely independent. Drift and beta error also widen the mid through `effective_steps`,
        making it the posterior mean -- separate from how far that can sit from the truth.
        """
        key = (option, signature)
        ...
        # Separate sources: moving both adversely assumes they conspire, overstating short-dated rates.
        ...
        # One source per company per parameter: their drift errors are independent, so never push together.
        for leg in option.legs:
            if leg.underlying_id == FED_FUNDS_RATE_UNDERLYING_ID:
                continue
    ...
    def _reference_prices(self, option: BinaryOption, values: dict[int, float], theo: float) -> tuple[float, float]:
        ...
        Whether a zero-step option settles on today's values or after one more move is unspecified, so
        bracket both rather than hand away what looks like free money.
        """
    ...
    def _build_quote(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        # The skew only modulates keenness: never bid above or offer below fair, so every fill pays.
        ...
        if self._unopposed():
            # A boundary market ties a 0.00/1.00 maker and the exchange splits the order; one penny
            # inside wins it outright, and fair value still bounds where we may step in.
    ...
    def _half_spread(self, uncertainty: float, counterparty_id: int, base: float, multiplier: float) -> float:
        ...
        # Toxicity may pass the model cap: against a persistent winner only a refusal-wide spread works.
    ...
    def _side_quantity(
        ...
        # The lot cap and the margin test do different jobs, and near a boundary only the lot cap
        # binds: a bid a penny off zero risks a penny a contract, so the budget affords hundreds of
        # lots while the cap allows twelve. Everything past it goes to whoever quotes behind us, and
        # a maker bidding 0.00 pays nothing for it. The budget already bounds the dollars, so let
        # the lot cap widen as the contract gets cheap.
        scale: float = self._UNOPPOSED_SIZE_MULTIPLE if self._unopposed() else 1.0
        scale *= _clamp(0.25 / max(margin_per_contract, _EPSILON), 1.0, self._MAX_CHEAP_MULTIPLE)
    ...
    def _evaluate_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:
        ...
        uncertainty: float = self._uncertainty(option, values, signature, theo)
        low_reference, high_reference = self._reference_prices(option, values, theo)
        # The fill-or-kill path uses its own multiplier: this width also sets the hurdle below.
        ...
        # Positive edge is not enough when margin is scarce, so it must also clear a return on the
        # margin it uses: eight contracts at 0.99 for a cent is one percent of half the balance.
    ...
    def _match_pending_fok(self, option_id: int, price: float, quantity: int) -> None:
        ...
        Contract and price alone cannot tell that fill from an ordinary request-for-quote fill on the
        same contract at the same penny, so the accepted quantity joins the key. An order is allocated
        at most in full, so requiring no more never rejects a genuine match.
        """
        for index, (pending_id, pending_price, expected_direction, pending_quantity) in enumerate(self._pending_fok):
            ...
            # Only a convention wrong from the very first trade is worth overturning: once any fill
            # agrees the reading is settled, and an inverted one buys every order it meant to sell.
    ...
    def _record_trade(self, option: BinaryOption, price: float, quantity: int, counterparty_id: int) -> None:
        ...
        # Shadow the exchange: each trade costs its maximum loss, never more optimistically.
        if quantity >= 0:
    ...
    def _update_markouts(self, settled_payoffs: dict[int, float]) -> None:
        """Score the day's trades against where the option stands one step later."""
    ...
    def _settle_expired(self, previous_values: dict[int, float]) -> dict[int, float]:
        ...
        for option_id in list(self._traded_contracts):
            ...
            # Each trade reserved its maximum loss, released separately -- netting would let a profitable
            # round trip destroy cash. `previous_payoff` is the exact credit and was tried alone: freed
            # margin raises every quote size and the grader fell 16.20 -> 16.00 on a falling market.
    ...
    def _adverse_selection_edge(self) -> float:
        """Widen when nobody is competing, or while our own fills are losing money.

        Per-counterparty toxicity sees neither: it needs a dozen trades against one name, and both of
        these are the whole session against everybody.
        """
        if self._unopposed():
    ...
    def _unopposed(self) -> bool:
        """Every decided auction won, so no rival has ever priced better and the spread is ours.

        The bar is every auction, not most: a 55% session cannot fake it, and if widening draws a
        competitor out, the first loss ends this for the rest of the session.
        """
    ...
    def _toxicity_edge(self, counterparty_id: int) -> float:
        ...
        if loss_per_contract <= 0.0:
            return 0.0
        # Charge more than the observed loss: a counterparty beating us this much keeps doing so.
    ...
    def _counterparty_pnl_per_contract(self, counterparty_id: int) -> float:
        ...
        contract in noise, drowning an edge of a few pennies, while the next day's theo is a
        conditional expectation. The mean alone still misjudges ordinary counterparties often enough
        to cost more than it saves, so this takes the optimistic end -- only a consistent loser is toxic.
        """
