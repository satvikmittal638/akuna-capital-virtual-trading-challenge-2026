...
# over n steps that term telescopes to rate_beta * (R_n - R_0) exactly, including the floor at zero
# where a down draw moves neither the rate nor `dR`. Conditional on the terminal rate alone,
# (log A_n, log T_n) is bivariate normal: mean log V_0 + n * drift + rate_beta * (r - R_0), per-step
# variance (sector_beta * sigma_S) ** 2 + idio ** 2, covariance sector_beta_A * sector_beta_T *
# sigma_S ** 2. Pricing is an exact Markov-chain DP over the terminal rate, then one normal CDF per
# rate state (a quadrature for two company legs). Only (drift, rate_beta, v_A, v_T, cov) matter and
# the rate chain needs only alpha_up = p_up + lam * target, alpha_down = p_down - lam * target,
# hence the bot's own `_Params`. The lone approximation is the per-step `round(value, 2)`.
...
# Prior on daily log drift. Positive rather than zero: these are growing frontier labs, and across
# warm-up histories estimates sit near half a percent a day, with about that much real dispersion.
_DRIFT_PRIOR_MEAN: float = 0.005
_DRIFT_PRIOR_STD_DEV: float = 0.008
# Prior on the two companies' return correlation. Both load on a common sector shock, so across
# warm-up histories the estimate sits high, near three quarters, varying little between markets.
_CORRELATION_PRIOR_MEAN: float = 0.75
_CORRELATION_PRIOR_STD_DEV: float = 0.20
# Prior on rate sensitivity, shrunk hard: no signal -- cross-case spread is all sampling noise (sd
# 0.029 vs mean standard error 0.037), and within a history the first half predicts the second at
# slope 0.08. Centred on the precision-weighted pooling, -0.024.
...
def _estimate_rate_process(
    ...
    # variation to regress against and the raw slope explodes. The penalty pulls toward a local random
    # walk -- what a flat sample evidences -- and fades once the history spans levels.
...
def _estimate_company_process(
    ...
    # Shrink correlation toward what this market structurally runs at: both load on one sector shock,
    # and the error goes into spread pricing, where var = v_A + v_T - 2 cov is acutely sensitive.
    ...
    # Variances come from as few as eight residuals, so the predictive law is Student t, not normal --
    # too thin in the tails, pushing every probability away from a half. Matching the t variance,
    # nu / (nu - 2), restores it; across the whole matrix, or correlation deflates with it.
    company = {
    ...
    # Shrink drift and rate beta, each toward its own prior with its own standard error. Twenty days of
    # a company up sixty percent implies two and a half percent a day, extrapolating a lucky run into
    # every price; a Bayesian prior barely touches a long history and hauls back a short one.
...
class MarketMaker:
    ...
    # Winning is itself bad news -- a counterparty lifts our offer exactly when our error made it look
    # cheap -- so above zero, but below one because fixed-width makers profit here. A log shows an
    # eleven-lot request lost with our bid 0.109 under fair: beaten on width, not size.
    ...
    # Fractions of capacity: contracts of typical margin the balance supports. At 0.30 -- six lots on
    # ten dollars against orders running to eleven -- a best-but-capped price sent the remainder down
    # the ladder to the next maker. The margin test below still binds.
    ...
    def __init__(
        ...
        # Every trade costs its maximum loss and only expiries pay it back, so sizes are margin-
        # driven and limits are set against capacity: contracts of half-dollar margin affordable.
    ...
    def _build_quote(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        # The skew only modulates aggressiveness: never bid above or offer below fair, so every
        # fill is expected to make money and inventory is managed by relative keenness alone.
    ...
    def _evaluate_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:
        ...
        # hurdle below, so the quote constant would recouple the two through the back door.
        ...
        # Positive edge is not enough when margin is scarce: eight contracts at 0.99 for a cent is
        # one percent on half the balance. So it must clear a return on the margin it consumes.
    ...
    def _settle_expired(self, previous_values: dict[int, float]) -> dict[int, float]:
        ...
        for option_id in list(self._traded_contracts):
            ...
            # Each trade reserved its own maximum loss, released separately -- netting would let a
            # profitable round trip destroy cash. `previous_payoff` is the exact credit and was tried
            # alone: the freed margin raises every quote size, and the grader fell 16.20 -> 16.00 on a
            # falling market. Under-margined is not the defect it looks.
    ...
    def _adverse_selection_edge(self) -> float:
        """Widen while this session's own fills have been losing money.

        Every remaining loss is a session where we traded and came out behind a maker that barely
        traded at all, so the width that wins flow is buying flow worth having less than it costs.
        Per-counterparty toxicity cannot see this -- it needs a dozen trades against one name, and
        this is the whole session against everybody -- so it reads the aggregate markout instead.
        """
