...
# `MarketParameters.advance_step`, is
#
#     d log(V) = drift + rate_beta * dR + sector_beta * S + eps
#
# where `dR` is the *realised* rate change. Over n steps that term telescopes to
# rate_beta * (R_n - R_0) exactly -- including the floor at zero, where a down draw leaves both
# the rate and `dR` unchanged -- so conditional on the terminal rate alone (log A_n, log T_n) is
...
# cov = sector_beta_A * sector_beta_T * sigma_S ** 2. Pricing is therefore an exact Markov-chain
# DP for the terminal rate, then one normal CDF per rate state (a quadrature for two company legs).
#
...
# alpha_down = p_down - lam * target, not the three separately. Hence the bot carries its own
# `_Params`. The one approximation is the per-step `round(value, 2)`, negligible against
# estimation error at realistic valuations.
...
class _Params:
    ...
    def effective_steps(self, num_steps: int, rate_change: float) -> float:
        ...
        covariance, which is the same multiple for both companies and for their covariance.
        """
...
def _estimate_rate_process(
    ...
    # points leaves almost no variation to regress against, and the raw slope then explodes --
    # implying one grid step swings the up probability by tens of points. The penalty pulls it
    # toward a local random walk, which is what a flat sample evidences, and fades once the
    # history genuinely spans levels.
...
def _estimate_company_process(
    ...
    # The variances are themselves estimated from as few as eight residuals. Treating them as known
    # makes the predictive law a normal when it is really a Student t -- too thin in the tails,
    # pushing every probability further from a half than it belongs. Matching the t's variance,
    # nu / (nu - 2), restores it, and it must scale the whole matrix or correlation is deflated too.
    company = {
    ...
    # Shrink the drift. Twenty days of a company rising sixty percent implies two and a half
    # percent a day, which taken literally extrapolates a lucky run into every option price. A
    # plain Bayesian prior leaves a long history nearly untouched and pulls a short trending one back.
    prior_variance: float = _DRIFT_PRIOR_STD_DEV**2
    total_weight: float = 0.0
    for underlying_id, params in company.items():
        standard_error_squared: float = mean_uncertainty[0] * params.variance
        weight: float = (
            prior_variance / (prior_variance + standard_error_squared)
            if standard_error_squared > _MIN_VARIANCE
            else 1.0
        )
        company[underlying_id] = replace(params, drift=_DRIFT_PRIOR_MEAN + (params.drift - _DRIFT_PRIOR_MEAN) * weight)
        total_weight += weight
    average_weight: float = total_weight / max(len(company), 1)
    mean_uncertainty = (mean_uncertainty[0] * average_weight, mean_uncertainty[1], mean_uncertainty[2])
...
class _CounterpartyStats:
    ...
    def markout_lower_bound(self, confidence_multiple: float) -> float:
        ...
        Using the *upper* end of the confidence interval for our own profit means a counterparty
        is only judged toxic when the losses are too consistent to be luck.
        """
...
class MarketMaker:
    # Counterparties here cross well away from fair value, so a nickel is closer to the
    # profit-maximising width than the penny-and-a-half a pure winner's-curse argument suggests:
    # quoting tighter mostly gives away edge on flow that would have traded anyway.
    _BASE_HALF_SPREAD: Final[float] = 0.05
    ...
    # fixed-width makers profit here, which says the flow is mostly benign.
    #
    # Two constants, not one: widening the market costs flow to a tighter maker, while demanding
    # more edge before crossing a fill-or-kill costs a trade that was there for the taking. Shared,
    # every experiment moved both at once and no result could be attributed. The quote side is the
    # lower -- a verbose log shows an eleven-lot request lost with our bid 0.109 under fair against
    # a competitor's 0.05, beaten on width not size. The fill-or-kill hurdle deliberately holds.
    ...
    # Both as fractions of capacity: how many contracts of typical margin the balance supports.
    _QUOTE_SIZE_FRACTION: Final[float] = 0.30
    ...
    def __init__(
        ...
        # selling -- the same convention the RFQ pipeline uses. `_match_pending_fok` watches the
        # sign of each resulting fill and flips this if the reading ever turns out to be wrong.
        self._pending_fok: list[tuple[int, float, int]] = []
    ...
    def _theo(
        self, option: BinaryOption, values: dict[int, float], signature: tuple[tuple[int, float], ...]
    ) -> float:
    ...
    def _uncertainty(
        ...
        Each source is pushed by one standard error and the resulting price moves are combined in
        quadrature, since the estimation errors are largely independent. Drift and beta error also
        widen the mid through `_Params.effective_steps` -- that makes the mid the posterior mean,
        which is a separate thing from how far that mean can sit from the truth.
        """
        key = (option, signature)
    ...
    def _build_quote(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        half_spread: float = self._half_spread(uncertainty, counterparty_id, self._QUOTE_UNCERTAINTY_MULTIPLIER)
    ...
    def _half_spread(self, uncertainty: float, counterparty_id: int, multiplier: float) -> float:
        model_spread: float = _clamp(
            self._BASE_HALF_SPREAD + multiplier * uncertainty,
            self._MIN_HALF_SPREAD,
            self._MAX_HALF_SPREAD,
        )
    ...
    def _evaluate_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:
        ...
        half_spread: float = self._half_spread(uncertainty, fok_order.counterparty_id, self._FOK_UNCERTAINTY_MULTIPLIER)
        skew: float = self._inventory_skew(option, theo, half_spread, values, signature)
        ...
        # A positive edge is not enough when margin is scarce: eight deep in-the-money contracts
        # at 0.99 for a cent apiece is a one percent return on half the balance, and that margin is
        # worth more quoting ordinary flow. So the trade must also clear a return on the margin it
        # consumes, with the hurdle rising as the balance is used up.
        ...
        self._pending_fok.append((fok_order.option_id, fok_order.price, direction))
        return True
    ...
    def _match_pending_fok(self, option_id: int, price: float, quantity: int) -> None:
        for index, (pending_id, pending_price, expected_direction) in enumerate(self._pending_fok):
            ...
            # Require repeated disagreement: an unrelated RFQ fill can look like a match.
            if self._fok_disagreements >= 2 and self._fok_disagreements > self._fok_agreements:
    ...
    def _settle_expired(self, previous_values: dict[int, float]) -> dict[int, float]:
        ...
        for option_id in list(self._traded_contracts):
            ...
            # here would make a profitable round trip destroy cash. Settlement may use the values
            # from before or after this advance, and crediting the smaller can only make the bot
            # more cautious, never insolvent. Crediting in full is exactly correct accounting and
            # was tried: the margin it frees raises every quote size and admits FOKs this declines,
            # and the grader fell 15.1 -> 14.3. Being under-margined is not the defect it looks.
    ...
    def _counterparty_pnl_per_contract(self, counterparty_id: int) -> float:
        ...
        day's own theo is a conditional expectation and moves far less. Even so the mean alone
        misjudges ordinary counterparties often enough to cost more than it saves, so what is
        returned is the optimistic end of the interval: only a consistent loser looks toxic.
        """
