...
def _estimate_company_process(
    ...
    # Shrink drift and rate beta toward their priors, each by its own standard error. Twenty days of a
    # company up sixty percent implies two and a half percent a day -- a lucky run put into every price.
...
class MarketMaker:
    # The floor width before uncertainty, and the dominant term. Split per path because `_half_spread`
    # also sets the fill-or-kill hurdle, so one constant moved both at once and hid every cause.
    ...
    def _build_quote(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        # A market at the boundary does not beat a maker quoting 0.00/1.00, it ties it, and the
        # exchange splits the order. One penny inside wins the whole thing instead of half, and the
        # guards keep that penny on the profitable side of fair -- so this never trades through theo,
        # and winning all of an order at better than fair beats winning half of it at fair.
    ...
    def _side_quantity(
        ...
        # Unopposed, these caps are the only thing leaving the rest of an order to whoever quotes
        # behind us: a bid five cents from zero costs a twentieth the margin of one at fair, so
        # margin stops binding and the cap alone decides how much of the request we take.
