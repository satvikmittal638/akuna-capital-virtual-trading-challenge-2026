...
class MarketMaker(_BaseMarketMaker):  # type: ignore[no-redef]
    ...
    Size is deliberately NOT coupled to this. Tying a size multiple to the same signal scored 14.30
    against 16.30: the width search itself converged fine, but multiplying position size 4x at the
    starting prior and 14.5x at full edge turned ordinary adverse selection into large losses --
    test 14 went +16.45 to -10.05, test 12 +24.92 to -0.72, total PnL 184 to 86.
    """

    _EDGE_STEP: Final[float] = 0.12
    _EDGE_DECAY: Final[float] = 0.5
    _MAX_EDGE_BONUS: Final[float] = 0.45
    _EDGE_SIZE_GAIN: Final[float] = 0.0
    ...
    def __init__(self, underlying_initial_state: list[Underlying],
        ...
        # No prior. The 0.10 lean cost flow in every competitive field for a gain only the one
        # uncontested session could collect.
        self._edge_bonus: float = 0.0
